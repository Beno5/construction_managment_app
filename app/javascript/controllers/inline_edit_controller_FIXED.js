...
// FIXED VERSION: Prevents double-save issues for SubTask inline-edit
//
// Key improvements:
// 1. Request lock prevents concurrent PATCH requests
// 2. Blur timeout is cleared when manually saving
// 3. Date inputs use 'change' event instead of 'blur' to prevent double-trigger
// 4. Debouncing for rapid field switching
// 5. Proper cleanup on disconnect to prevent re-initialization issues
//
// Usage: (same as before, no changes needed in HTML)
//   <div data-controller="inline-edit"
//        data-inline-edit-model-value="sub_task"
//        data-inline-edit-field-value="planned_start_date"
//        data-inline-edit-type-value="date"
//        data-inline-edit-url-value="/businesses/1/projects/2/tasks/3/sub_tasks/4"
//        data-inline-edit-original-value="<%= sub_task.planned_start_date %>"
//        data-inline-edit-record-updated-at-value="<%= sub_task.updated_at.iso8601 %>"
//        data-action="click->inline-edit#activate">
//     <%= I18n.l(sub_task.planned_start_date) if sub_task.planned_start_date %>
//   </div>
...
export default class extends Controller {
  ...
  connect() {
    ...
    // 🔥 FIX 1: Request lock to prevent concurrent saves
    this.isSaving = false

    // 🔥 FIX 2: Store blur timeout so we can clear it
  ...
  disconnect() {
    // 🔥 FIX 3: Clear all pending timeouts on disconnect
  ...
  async activate(event) {
    ...
    // 🔥 FIX 4: If another field is being edited, save it first before activating this one
    if (window.__inlineEditActiveElement && window.__inlineEditActiveElement !== this.element) {
      ...
      if (activeController) {
        // 🔥 FIX 5: Clear the blur timeout of the active field to prevent double-save
        if (activeController.blurTimeout) {
          clearTimeout(activeController.blurTimeout)
          activeController.blurTimeout = null
        }
  ...
  createInput() {
    ...
    switch (this.typeValue) {
      ...
      case 'date':
        // 🔥 FIX 6: Date inputs use CHANGE event instead of BLUR to prevent double-trigger
        input = document.createElement('input')
        input.type = 'date'
        input.value = this.originalValue
        input.className = 'w-full px-2 py-1 text-sm border-2 border-blue-500 rounded focus:outline-none focus:ring-2 focus:ring-blue-300 dark:bg-gray-700 dark:text-white dark:border-blue-400'

        // Date-specific events: change + keydown only (NO blur)
        input.addEventListener('change', () => {
          console.log('Date change event fired')
          this.save()
        })
        input.addEventListener('keydown', (e) => this.handleKeydown(e))
        // Do NOT add blur listener for date inputs
        break
    ...
    // Add event listeners (select and date already have their own listeners added in their cases)
    if (this.typeValue !== 'select' && this.typeValue !== 'date') {
      input.addEventListener('keydown', (e) => this.handleKeydown(e))
      input.addEventListener('blur', () => this.handleBlur())
    }
  ...
  handleKeydown(event) {
    if (event.key === 'Enter' && this.typeValue !== 'textarea') {
      event.preventDefault()
      // 🔥 FIX 7: Clear blur timeout before manual save
      if (this.blurTimeout) {
        clearTimeout(this.blurTimeout)
        this.blurTimeout = null
      }
  ...
  handleBlur() {
    // 🔥 FIX 8: Store timeout reference so it can be cleared
    // Delay blur to allow click events to fire first
    this.blurTimeout = setTimeout(() => {
      if (this.isEditing && !this.isSaving) {
        console.log('Blur timeout fired, saving...')
        this.save()
      }
      this.blurTimeout = null
    }, 200)
  ...
  async save() {
    ...
    // 🔥 FIX 9: Request lock - prevent concurrent saves
    if (this.isSaving) {
    ...
    try {
      ...
      console.log('Sending PATCH request:', this.urlValue, payload)
      ...
      // 🔥 FIX 11: Always clear saving flag in finally block
      this.isSaving = false
    }
  }
  ...
  cancel() {
    ...
    // 🔥 FIX 12: Clear blur timeout on cancel
    if (this.blurTimeout) {
      clearTimeout(this.blurTimeout)
      this.blurTimeout = null
    }
